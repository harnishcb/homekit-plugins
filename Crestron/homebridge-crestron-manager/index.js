...
//-- List of Active Sockets & Types --//
let active_socket = [];
let plugin_socket = [];
let plugin_socket_type = [];


//-- List of Device ID range --//
let command_list = [];
let fb_fan_range = [];
let fb_dimmer_range = [];
...
/*
Server for Crestron to Connect
*/
setTimeout(() => {
  const server = net.createServer((socket) => {
    ...
    socket.on('data', (data) => {
      data = data.toString();
      log.info('Data received from Crestron Processor:', data);
      let match_type = []
  ...
  server.listen(36500, () => {
    log.info(`Server for Crestron listening on port 36500`);
  });
...
/*
Server for the Plugins to Connect
*/
const server_plugin = net.createServer((socket) => {
  ...
  socket.on('data', (data) => {
    ...
    else if (data.indexOf('LockID') !== -1){
      fb_ac_range = data.split(':')[1].split(',');
      plugin_socket_type[plugin_socket.indexOf(socket)] = 'Lock';
    }
    else {
