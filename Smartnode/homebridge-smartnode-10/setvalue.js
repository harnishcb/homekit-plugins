...
var client = net.Socket();
	var temp_data = ["XX","XX","XX","XX","XX","XX","XX","XX","XX","XX"];
	var temp_val = [255,255,255,255,255,255,255,255,255,255];
	for(var i=0;i<test_que[gen].length; i++){
		...
		if(test_que[gen][i][0] == 'D'){
			var current_value = parseInt(test_que[gen][i].slice(2,));
			temp_val[parseInt(test_que[gen][i][1])] = current_value;
			if(100 >= current_value && current_value > 0){
			  temp_data[parseInt(test_que[gen][i][1])] = "AX";
			}
			else {
			 temp_data[parseInt(test_que[gen][i][1])] = "0X";
			}
		}
	}
