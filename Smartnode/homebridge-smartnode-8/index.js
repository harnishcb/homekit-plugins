...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-smartnode-8", "Smartnode-Device-8_Channel", sn);
}
...
updated_token.push(this.token);
 ...
 client.connect(13002, this.ip, function() {
	console.log('Connected to SN-8 at IP='+ip);
	auto_fetch_status();
 ...
 client.on('data', function(data) {
	...
	console.log('Received from device: '+'cmd_type='+data.cmd+', val='+data.val+' Dimmer='+data.dimmer+' ip='+ip);			
		for(var i=0;i<data.val.length;i++){
		  ...
		  current_state[(gen*8)+i] = temp_1;
		  current_value[(gen*8)+i] = data.dimmer[i];
		}
	}
	if(data.cmd == "SET"){
		...
		current_state[(gen*8)+data.node-1] = temp_1;
	    current_value[(gen*8)+data.node-1] = data.dimmer;
	}
	if(data.cmd == "CUS" && data.data != "undefined"){
		console.log('Received: '+'cmd_type=CUS, val='+data.val+' Dimmer='+data.data+' ip='+ip);
	   }
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4,this.conf[4].Channel_5,this.conf[5].Channel_6,this.conf[6].Channel_7,this.conf[7].Channel_8];
...
class SNSWITCH{
	...
	this.gen = gen;
	this.ip = ip;
    this.index = ((this.gen*8) + Number(this.node)-1);
	this.name =  "SN8-"+name+'-'+node;
	 ...
	 getServices(){
      var infoService = new Service.AccessoryInformation();
        infoService
         ...
         .setCharacteristic(Characteristic.Model, "SN8-Switch")
   ...
   setsnbulb(stt){
	...
	getServices(){
      var infoService = new Service.AccessoryInformation();
        infoService
         .setCharacteristic(Characteristic.Manufacturer, "Smartnode Automations")
         .setCharacteristic(Characteristic.Model, "SN8-Dimmer")
         .setCharacteristic(Characteristic.SerialNumber,this.serial);
      this.SNDIMMER
...
class SNFAN{
	...
	getServices(){
      var infoService = new Service.AccessoryInformation();
        infoService
         .setCharacteristic(Characteristic.Manufacturer, "Smartnode Automations")
         .setCharacteristic(Characteristic.Model, "SN8-Fan")
         .setCharacteristic(Characteristic.SerialNumber, this.serial);
     this.SNFAN
...
class SNCCT{
	...
	getServices(){
      var infoService = new Service.AccessoryInformation();
        infoService
         .setCharacteristic(Characteristic.Manufacturer, "Smartnode Automations")
         .setCharacteristic(Characteristic.Model, "SN-8")
         .setCharacteristic(Characteristic.SerialNumber,this.serial);
      this.SNCCT
        .getCharacteristic(Characteristic.On).onGet(this.getsnbulb.bind(this)).onSet(this.setsnbulb.bind(this));
      this.SNCCT
