...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-smartnode-4", "Smartnode-Device-4_Channel", sn);
}  
...
sn.prototype.accessories = function(callback){
	...
	console.log('Connected to SN-4 at IP='+ip);
	auto_fetch_status();
	 ...
	 data = JSON.parse(data);
	  ...
	  console.log('Received: '+'cmd_type=STS, val='+data.val+' Dimmer='+data.dimmer+' ip='+ip);
		for(var i=0;i<data.val.length;i++){
		  ...
		  current_state[(gen*4)+i] = temp_1;
		  current_value[(gen*4)+i] = data.dimmer[i];
      }
	}
	if(data.cmd == "SET"){
	    console.log('Received: Node='+data.node+'  cmd_type=SET, cmd_source='+data.by+' val='+data.val+' Dimmer='+data.dimmer+' ip='+ip);					
		  ...
		  current_state[(gen*4)+data.node-1] = temp_1;
	      current_value[(gen*4)+data.node-1] = Number(data.dimmer);
	}
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4];
...
for(var i=0;i<4;){
	switch(type_locate[i]){
	  ...
	  console.log("Final List ",update_id);
 }
 callback(results)
}
