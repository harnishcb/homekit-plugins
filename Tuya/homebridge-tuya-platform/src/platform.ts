...
export class TuyaPlatform implements DynamicPlatformPlugin {
  ...
  async initCustomProject() {
    ...
    if (res.success === false) {
      ...
      this.log.info("Restarting the HB service");
      exec("hb-service restart", (error, stdout, stderr) => {
        if (error) {
          this.log.error(`Error executing command: ${error.message}`);
        }
        this.log.info('HB service restart status:',stdout);
        if (stderr) {
          this.log.error('Command errors:',stderr);
        }
      });
