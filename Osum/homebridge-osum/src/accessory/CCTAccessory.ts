...
export class CCTAccessory {
  ...
  constructor(
    ...
    this.service.getCharacteristic(this.platform.Characteristic.ColorTemperature)
      .onSet(this.setCCT.bind(this))      // SET - bind to the 'setBrightness` method below
      .onGet(this.getCCT.bind(this))
      .setProps({
        minValue: 142,
        maxValue: 500,
        minStep: 1,
    ...
    myEmitter.on('Feedback'+device.rmacid+device.bus.toString()+device.nk.toString(), (data) => {
      this.platform.log.info('Data received for CCT Accessory:', data.toString());
  ...
  async setCCT(value: CharacteristicValue) {
    this.current_state.ColorTemperature = value as number;
    this.platform.log.info('Set Characteristic Color Temperature -> ', value);
    value = (Math.round(1000000/Number(value))).toString();
    const response: any = await this.api._SetDevice(this.device, value.toString(), 'SetCT');
    this.platform.log.info('Response Received from Osum server:', JSON.stringify(response.data));
  }

  async getCCT(): Promise<CharacteristicValue> {
    const isOn = Math.round(1000000/this.current_state.ColorTemperature);
    this.platform.log.info('Get Characteristic Color Temperature ->', isOn);
    return isOn;
  }
