...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_11", "Zemote_Panel-11", zm);
}  
...
// console.log(current_value);
 var len = data.lastIndexOf('.')-11;
// console.log('Update 11 channel called');
 for(var i=0;i<11;i++){
	 if(execute[i]){
	// if(execute[(gen*11)+i]){
        // console.log('11-',gen, i);
	 current_value[i] = data[len+i];
	 results[i].updatevalue(data[len+i]);
	// current_value[(gen*11)+i] = data[len+i];
	 }
 }
 client.destroy();
 });
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4,this.conf[4].Channel_5,this.conf[5].Channel_6,this.conf[6].Channel_7,this.conf[7].Channel_8,this.conf[8].Channel_9,this.conf[9].Channel_10,this.conf[10].Channel_11];
...
class ZMSWITCH{
	...
	setswitch(stt){ 
      ...
      execute[this.node] = false;
	  setTimeout(() =>  { execute[this.node] = true } , 5000);
      this.sent.set(this.token+'#%'+('aaaaaaaaaaa').substring(0, this.node) + String(Number(stt)) + ('aaaaaaaaaaa').substring(this.node + 1) , this.ip, this.gen);
    }	
	getswitch(){   	
...
class ZMDIMMER{
   ...
   setswitch(stt){
	  ...
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaaa').substring(0, this.node) + '0' + ('aaaa').substring(this.node + 1) , this.ip);		  
	  }
	  else{
  ...
  constructor(token, node, position,ip,sent,gen,name){
	...
	this.name =  "ZM Fan-"+name+node;
	 ...
	 setswitch(stt){
	  ...
	  setTimeout(() =>  { execute[this.position] = true } , 5000); 	 
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaaaaaaaaaa').substring(0, this.node) + '0' + ('aaaaaaaaaaa').substring(this.node + 1) , this.ip,this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaaaaaaa').substring(0, this.node) + current_value[this.position] + ('aaaaaaaaaaa').substring(this.node + 1) , this.ip,this.gen),500);
	  }
          }
         setvalue(stt){
           current_value[this.position] = fan_map[stt/20];
	         setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaaaaaaa').substring(0, this.node) + current_value[this.position] + ('aaaaaaaaaaa').substring(this.node + 1) , this.ip,this.gen),500);
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaaaaaaaaaaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);		  
