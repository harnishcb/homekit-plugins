...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_6", "Zemote_Panel-6", zm);
}  
...
var curtain_sent = 	require('./setcurtain.js');
 ...
 data = data.toString();
 var len = data.lastIndexOf('.')-6;
 for(var i=0;i<6;i++){
	 if(execute[(gen*5)+i]){
	 results[i].updatevalue(data[len+i]);
	 current_value[(gen*5)+i] = data[len+i];
	 }
 }
 client.destroy();
 });
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4,this.conf[4].Channel_5,this.conf[5].Channel_6,this.conf[7].Curtain_1,this.conf[8].Curtain_2];
	...
	switch(type_locate[i]){
		case "Fan":
		 ...
		 case "Switch":
		   ...
		   break;	
		 case "Yes":	
		   results.push(new ZMCURTAIN(this.token,i-6,this.ip,curtain_sent,this.name));
		   current_value.push("0"); 
		   execute.push(true);		 
		   break;		 
}  
...
class ZMSWITCH{
	...
	setswitch(stt){ 
      ...
      this.sent.set(this.token+'#%'+('aaaaaa').substring(0, this.node) + String(Number(stt)) + ('aaaaaa').substring(this.node + 1) , this.ip, this.gen);
    }	
	getswitch(){   	
	 ...
	 getServices(){
       var infoService = new Service.AccessoryInformation();
        infoService
         ...
         .setCharacteristic(Characteristic.Model, "ZM-6")
...
class ZMDIMMER{
   ...
   setswitch(stt){
      execute[this.position] = false;
	  setTimeout(() =>  { execute[this.position] = true } , 5000);	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaaaaa').substring(0, this.node) + '0' + ('aaaaaa').substring(this.node + 1) , this.ip);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaa').substring(0, this.node) + current_value[this.position] + ('aaaaaa').substring(this.node + 1) , this.ip, this.gen),300);
	  }
     }
     setvalue(stt){	 
...
class ZMFAN{
    ...
    this.ZMFAN = new Service.Fan(this.name);
	 ...
	 setswitch(stt){
	  ...
	  setTimeout(() =>  { execute[this.position] = true } , 5000); 	 
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaaaaa').substring(0, this.node) + '0' + ('aaaaaa').substring(this.node + 1) , this.ip,this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaa').substring(0, this.node) + current_value[this.position] + ('aaaaaa').substring(this.node + 1) , this.ip,this.gen),500);
	  }
          }
         setvalue(stt){
           current_value[this.gen*6] = fan_map[stt/20];
	} 	 
	getswitch(){
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaaaaaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);		
