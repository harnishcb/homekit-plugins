...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_3", "Zemote_Panel-3", zm);
}  
...
var curtain_sent = require('./setcurtain.js');
 ...
 data = data.toString();
 var len = data.lastIndexOf('.')-3;
 for(var i=0;i<3;i++){
	 if(execute[(gen*3)+i]){
	 results[i].updatevalue(data[len+i]);
	 current_value[(gen*3)+i] = data[len+i];
	 }
 }
 client.destroy();
 });
...
setInterval(() => { getupdate() }, 750);

var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Curtain_1,this.conf[4].Curtain_2];
	...
	switch(type_locate[i]){
		case "Fan":
		 ...
		 case "Switch":
		   ...
		   break;	
		 case "Yes":	
		   results.push(new ZMCURTAIN(this.token,i-3,this.ip,curtain_sent,this.name));
		   current_value.push("0"); 
		   execute.push(true);		 
		   break;			 
	} 	 
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);		  
