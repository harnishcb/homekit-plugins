...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_2", "Zemote_Panel-2", zm);
}  
...
var curtain_sent = require('./setcurtain.js');
 ...
 client.connect(50505, ip, function() {
	client.write(token+"#%aa");
 });
 client.on('data', function(data) {
 data = data.toString();
 var len = data.lastIndexOf('.')-2;
 for(var i=0;i<2;i++){
	 if(execute[(gen*2)+i]){
	 results[i].updatevalue(data[len+i]);
	 current_value[(gen*2)+i] = data[len+i];
	 }
 }
 client.destroy();
 });
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Curtain_1,this.conf[3].Curtain_2];
	...
	switch(type_locate[i]){
		case "Fan":
		 ...
		 case "Switch":
		   ...
		   break;
		 case "Yes":	
		   results.push(new ZMCURTAIN(this.token,i-2,this.ip,curtain_sent,this.name));
		   current_value.push("0"); 
		   execute.push(true);		 
		   break;	  			 
	} 	 
...
class ZMSWITCH{
	...
	setswitch(stt){ 
      ...
      this.sent.set(this.token+'#%'+('aa').substring(0, this.node) + String(Number(stt)) + ('aa').substring(this.node + 1) , this.ip, this.gen);
    }	
	getswitch(){   	
	 ...
	 getServices(){
       var infoService = new Service.AccessoryInformation();
        infoService
         ...
         .setCharacteristic(Characteristic.Model, "LFM-7")
...
class ZMDIMMER{
   ...
   setswitch(stt){
	  ...
	  setTimeout(() =>  { execute[this.position] = true } , 5000);	  
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aa').substring(0, this.node) + '0' + ('aa').substring(this.node + 1) , this.ip,this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aa').substring(0, this.node) + current_value[this.position] + ('aa').substring(this.node + 1) , this.ip, this.gen),300);
	  }
     }
     setvalue(stt){	 
      current_value[this.position] = dimmer_map[Math.round(stt/(20/3))];
      setTimeout(() => this.sent.set(this.token+'#%'+('aa').substring(0, this.node) + current_value[this.position] + ('aa').substring(this.node + 1) , this.ip, this.gen),200);	     
	 } 	 
	getswitch(){
...
class ZMFAN{
    ...
    this.ZMFAN = new Service.Fan(this.name);
	 ...
	 setswitch(stt){
      ...
      setTimeout(() => this.sent.set(this.token+'#%'+('aa').substring(0, this.node) + current_value[this.position] + ('aa').substring(this.node + 1) , this.ip, this.gen),400);		 
	} 	 
	getswitch(){
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);		  
