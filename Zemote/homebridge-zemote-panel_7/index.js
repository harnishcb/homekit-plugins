...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_7", "Zemote_Panel-7", zm);
}  
...
var curtain_sent = 	require('./setcurtain.js');
 ...
 data = data.toString();
 var len = data.lastIndexOf('.')-7;
 for(var i=0;i<7;i++){
	 if(execute[(gen*7)+i]){
	 results[i].updatevalue(data[len+i]);
	 current_value[(gen*7)+i] = data[len+i];
	 }
 }
 client.destroy();
 });
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4,this.conf[4].Channel_5,this.conf[5].Channel_6,this.conf[6].Channel_7,this.conf[7].Curtain_1,this.conf[8].Curtain_2];
	...
	switch(type_locate[i]){
		case "Fan":
		 ...
		 case "Switch":
		   ...
		   break;	
		 case "Yes":	
		   results.push(new ZMCURTAIN(this.token,i-7,this.ip,curtain_sent,this.name));
		   current_value.push("0"); 
		   execute.push(true);		 
		   break;		 
}  
...
class ZMSWITCH{
	...
	setswitch(stt){ 
      ...
      this.sent.set(this.token+'#%'+('aaaaaaa').substring(0, this.node) + String(Number(stt)) + ('aaaaaaa').substring(this.node + 1) , this.ip, this.gen);
    }	
	getswitch(){   	
...
class ZMFAN{
	constructor(token, node, position,ip,sent,gen,name){
	 ...
	 setswitch(stt){
	  ...
	  setTimeout(() =>  { execute[this.position] = true } , 5000); 	 
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaaaaaa').substring(0, this.node) + '0' + ('aaaaaaa').substring(this.node + 1) , this.ip,this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaaa').substring(0, this.node) + current_value[this.position] + ('aaaaaaa').substring(this.node + 1) , this.ip,this.gen),500);
	  }
          }
         setvalue(stt){
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaaaaaaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);		  
