...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_5", "Zemote_Panel-5", zm);
}  
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4,this.conf[4].Channel_5,this.conf[5].Curtain_1,this.conf[6].Curtain_2];
	...
	switch(type_locate[i]){
		case "Fan":
		 ...
		 case "Switch":
		   ...
		   break;	
	     case "Yes":	
		   results.push(new ZMCURTAIN(this.token,i-5,this.ip,curtain_sent,this.name));
		   current_value.push("0"); 
		   execute.push(true);		 
		   break;	 
}
...
class ZMSWITCH{
	...
	setswitch(stt){ 
      ...
      this.sent.set(this.token+'#%'+('aaaaa').substring(0, this.node) + String(Number(stt)) + ('aaaaa').substring(this.node + 1) , this.ip, this.gen);
    }	
	getswitch(){   	
...
class ZMDIMMER{
   ...
   setswitch(stt){
	  ...
	  setTimeout(() =>  { execute[this.position] = true } , 5000);	  
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaaaa').substring(0, this.node) + '0' + ('aaaaa').substring(this.node + 1) , this.ip, this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaa').substring(0, this.node) + current_value[this.position] + ('aaaaa').substring(this.node + 1) , this.ip, this.gen),300);
	  }
     }
     setvalue(stt){	 
...
class ZMFAN{
    ...
    this.ZMFAN = new Service.Fan(this.name);
	 ...
	 setswitch(stt){
      ...
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaa').substring(0, this.node) + current_value[this.position] + ('aaaaa').substring(this.node + 1) , this.ip,this.gen),1200);
	  }
          }
         setvalue(stt){
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaaaaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);		  
