...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_1", "Zemote_Panel-1", zm);
}  
...
var curtain_sent = require('./setcurtain.js');
 ...
 client.connect(50505, ip, function() {
	client.write(token+"#%a");
 });
 client.on('data', function(data) {
 data = data.toString();
 var len = data.lastIndexOf('.')-1;
 for(var i=0;i<1;i++){
	 if(execute[(gen)+i]){
	 results[i].updatevalue(data[len+i]);
	 current_value[(gen)+i] = data[len+i];
	 }
 }
 client.destroy();
 });
...
var type_locate = [this.conf[0].Channel_1,this.conf[0].Total_Curtains];
 ...
 for(var i=0;i<type_locate[type_locate.length-1];i++){
	results.push(new ZMCURTAIN(this.token,i,this.ip,curtain_sent,this.name));
 }
 callback(results)
}
...
class ZMSWITCH{
	...
	setswitch(stt){
      ...
      this.sent.set(this.token+'#%'+('aaa').substring(0, this.node) + String(Number(stt)) + ('aaa').substring(this.node + 1) , this.ip, this.gen);
    }
	getswitch(){
	 ...
	 getServices(){
       var infoService = new Service.AccessoryInformation();
        infoService
         ...
         .setCharacteristic(Characteristic.Model, "ZM-1")
...
class ZMDIMMER{
   ...
   setswitch(stt){
	  ...
	  setTimeout(() =>  { execute[this.position] = true } , 5000);	  
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaa').substring(0, this.node) + '0' + ('aaa').substring(this.node + 1) , this.ip,this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaa').substring(0, this.node) + current_value[this.position] + ('aaa').substring(this.node + 1) , this.ip, this.gen),300);
	  }
     }
     setvalue(stt){
...
class ZMFAN{
    ...
    this.ZMFAN = new Service.Fan(this.name);
	 ...
	 setswitch(stt){
	  ...
	  setTimeout(() =>  { execute[this.position] = true } , 5000); 	 
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaa').substring(0, this.node) + '0' + ('aaaa').substring(this.node + 1) , this.ip,this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaa').substring(0, this.node) + current_value[this.position] + ('aaaa').substring(this.node + 1) , this.ip, this.gen),500);
	  }
          }
         setvalue(stt){
           current_value[this.gen*3] = fan_map[stt/20];
	} 	 
	getswitch(){
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aX'+ this.node + String(2-(stt/100)) + '999' , this.ip);
	 ...
	 getServices(){
		var infoService = new Service.AccessoryInformation();
		  infoService
		   ...
		   .setCharacteristic(Characteristic.SerialNumber, this.serial);
	   this.ZMCURTAIN
		   .getCharacteristic(Characteristic.TargetPosition).onSet(this.settp.bind(this)).onGet(this.getcp.bind(this))
		   .setProps({
			minValue: 0,
			maxValue: 100,
			minStep: 100
