...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_9", "Zemote_Panel-9", zm);
}  
...
var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4,this.conf[4].Channel_5,this.conf[5].Channel_6,this.conf[6].Channel_7,this.conf[7].Channel_8,this.conf[8].Channel_9,this.conf[9].Curtain_1,this.conf[10].Curtain_2];
...
class ZMSWITCH{
	...
	setswitch(stt){ 
      ...
      this.sent.set(this.token+'#%'+('aaaaaaaaa').substring(0, this.node) + String(Number(stt)) + ('aaaaaaaaa').substring(this.node + 1),this.ip,this.gen);
    }	
	getswitch(){   	
	 ...
	 getServices(){
       var infoService = new Service.AccessoryInformation();
        infoService
         ...
         .setCharacteristic(Characteristic.Model, "ZM-9")
...
class ZMDIMMER{
    ...
    this.ZMDIMMER = new Service.Lightbulb(this.name);
	 ...
	 setswitch(stt){
	  ...
	  if(stt == false){
      this.sent.set(this.token+'#%'+('aaaaaaaaa').substring(0, this.node) + '0' + ('aaaaaaaaa').substring(this.node + 1) , this.ip,this.gen);		  
	  }
	  else{
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaaaaa').substring(0, this.node) + current_value[this.node] + ('aaaaaaaaa').substring(this.node+1) , this.ip),500);
	  }
          }
         setvalue(stt){
           current_value[this.node] = dimmer_map[stt];
	         setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaaaaa').substring(0, this.node) + current_value[this.node] + ('aaaaaaaaa').substring(this.node+1) , this.ip),500);

	} 	 
	getswitch(){
           return Boolean(parseInt(dimmer_map.indexOf(current_value[this.node])));	
	}	
	getvalue(){
           return dimmer_map.indexOf(current_value[this.node]);	
	}
	...
	updatevalue(cs){
	  ...
	  this.ZMDIMMER.getCharacteristic(Characteristic.Brightness).updateValue(parseInt(dimmer_map.indexOf(cs)));
    }
}
...
class ZMFAN{
    ...
    setswitch(stt){
      ...
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaaaaaaa').substring(0, this.node) + current_value[this.node] + ('aaaaaaaaa').substring(this.node + 1) , this.ip,this.gen),500);
	  }
          }
    setvalue(stt){
	...
	updatevalue(cs){
	  this.ZMFAN.getCharacteristic(Characteristic.On).updateValue(Boolean(fan_map.indexOf(cs)));
	  this.ZMFAN.getCharacteristic(Characteristic.RotationSpeed).updateValue(fan_map.indexOf(cs)*20);
    }
}
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.ZMCURTAIN = new Service.WindowCovering(this.name);
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaaaaaaaaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);		  
