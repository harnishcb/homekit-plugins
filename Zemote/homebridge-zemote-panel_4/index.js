...
module.exports = (homebridge) => {
    ...
    homebridge.registerPlatform("homebridge-zemote-panel_4", "Zemote_Panel-4", zm);
}  
...
var curtain_sent = 	require('./setcurtain.js');
 ...
 data = data.toString();
 var len = data.lastIndexOf('.')-4;
 for(var i=0;i<4;i++){
	 if(execute[(gen*4)+i]){
	 results[i].updatevalue(data[len+i]);
	 current_value[(gen*4)+i] = data[len+i];
	 }
 }
 client.destroy();
 });
...
setInterval(() => { getupdate() }, 1500);
 ...
 var type_locate = [this.conf[0].Channel_1,this.conf[1].Channel_2,this.conf[2].Channel_3,this.conf[3].Channel_4,this.conf[4].Curtain_1,this.conf[5].Curtain_2];
...
class ZMSWITCH{
	...
	setswitch(stt){ 
      ...
      this.sent.set(this.token+'#%'+('aaaa').substring(0, this.node) + String(Number(stt)) + ('aaaa').substring(this.node + 1) , this.ip, this.gen);
    }	
	getswitch(){   	
   ...
   setswitch(stt){
	...
	if(stt == false){
     ...
     setvalue(stt){	 
      ...
      setTimeout(() => this.sent.set(this.token+'#%'+('aaaa').substring(0, this.node) + current_value[this.position] + ('aaaa').substring(this.node + 1) , this.ip,this.gen),200);	     
	 } 	 
	getswitch(){
  ...
  constructor(token, node, position,ip,sent,gen,name){
    ...
    this.ZMFAN = new Service.Fan(this.name);
	 ...
	 setswitch(stt){
      ...
      setvalue(stt){
           current_value[this.gen*4] = fan_map[stt/20];
...
class ZMCURTAIN{
	constructor(token,node,ip,sent,name){
	  ...
	  this.status = 100;
	   ...
	   settp(stt){
		this.sent.set(this.token+'#%aaaaX'+ this.node + String(2-(stt/100)) + '600' , this.ip);	
		this.status = stt;	  
