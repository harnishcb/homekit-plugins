...
module.exports = {
  ...
  0x0003: {
    parse: function(buffer) {
      ...
      for (var i = 3, length = buffer.length; i < length; i++) {
        ...
        for (var n = 0; n < 8 && channel < channels.length; n++)
          channels[channel++] = {
            number: channel,
            status: !!(byte & (1 << n))
          };
    ...
    encode: function(data) {
      ...
      for (var i = 0; i < bytes; i++) {
        ...
        for (var n = 0; n < 8 && channel < length; n++)
          if (channels[channel++].status) byte |= 1 << n;
  ...
  0xEFFF: {
    parse: function(buffer) {
      var areas = new Array(buffer.readUInt8(0));
      var channels = new Array(buffer.readUInt8(areas.length + 1));

      var i;

      for (i = 0; i < areas.length; i++) areas[i] = {
        number: i + 1,
      ...
      var data = { areas: areas, channels: channels };
  ...
  0x15CE: {
    ...
    encode: function(data) {
      ...
      buffer.writeUInt8(data.area, 0);
      buffer.writeUInt8(data.switch, 1);
  ...
  0x15CF: {
    parse: function(buffer) {
      var data = {
        area: buffer.readUInt8(0),
        switch: buffer.readUInt8(1),
        contact: Boolean(buffer.readUInt8(2))
      };
      return data;
    },
    ...
    encode: function(data) {
      ...
      buffer.writeUInt8(data.area, 0);
      buffer.writeUInt8(data.switch, 1);
      buffer.writeUInt8(data.contact ? 1 : 0, 2);
  ...
  // XX.1.2 Panel brightness/lock response
  0xE013: {
    parse: function(buffer) {
      return { success: status.parse(buffer, 0) };
    },
